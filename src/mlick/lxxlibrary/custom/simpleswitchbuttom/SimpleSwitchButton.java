package mlick.lxxlibrary.custom.simpleswitchbuttom;

import mlick.lxxlibrary.R;
import mlick.lxxlibrary.custom.simpleswitchbuttom.AnimationController.OnAnimateListener;
...
public class SimpleSwitchButton extends CompoundButton {
	...
	@SuppressLint("NewApi")
	public SimpleSwitchButton(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initView();

		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SimpleSwitchButton);

		mConf.setThumbMarginInPixel(ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_thumb_margin, mConf.getDefaultThumbMarginInPixel()));
		mConf.setThumbMarginInPixel(ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_thumb_marginTop, mConf.getThumbMarginTop()),
				ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_thumb_marginBottom, mConf.getThumbMarginBottom()),
				ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_thumb_marginLeft, mConf.getThumbMarginLeft()),
				ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_thumb_marginRight, mConf.getThumbMarginRight()));
		mConf.setRadius(ta.getInt(R.styleable.SimpleSwitchButton_radius, Configuration.Default.DEFAULT_RADIUS));

		mConf.setThumbWidthAndHeightInPixel(ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_thumb_width, -1), ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_thumb_height, -1));

		mConf.setMeasureFactor(ta.getFloat(R.styleable.SimpleSwitchButton_measureFactor, -1));

		mConf.setInsetBounds(ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_insetLeft, 0), ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_insetTop, 0),
				ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_insetRight, 0), ta.getDimensionPixelSize(R.styleable.SimpleSwitchButton_insetBottom, 0));

		int velocity = ta.getInteger(R.styleable.SimpleSwitchButton_animationVelocity, -1);
	...
	private void fetchDrawableFromAttr(TypedArray ta) {
		...
		mConf.setOffDrawable(fetchDrawable(ta, R.styleable.SimpleSwitchButton_offDrawable, R.styleable.SimpleSwitchButton_offColor, Configuration.Default.DEFAULT_OFF_COLOR));
		mConf.setOnDrawable(fetchDrawable(ta, R.styleable.SimpleSwitchButton_onDrawable, R.styleable.SimpleSwitchButton_onColor, Configuration.Default.DEFAULT_ON_COLOR));
	...
	private Drawable fetchThumbDrawable(TypedArray ta) {
		...
		Drawable tempDrawable = ta.getDrawable(R.styleable.SimpleSwitchButton_thumbDrawable);
		if (tempDrawable != null) {
			return tempDrawable;
		}

		int normalColor = ta.getColor(R.styleable.SimpleSwitchButton_thumbColor, Configuration.Default.DEFAULT_THUMB_COLOR);
		int pressedColor = ta.getColor(R.styleable.SimpleSwitchButton_thumbPressedColor, Configuration.Default.DEFAULT_THUMB_PRESSED_COLOR);
